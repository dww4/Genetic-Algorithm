...
void Eval(){
  double y,x,a,b;
  int i,j,count;
  double factor = 100000000; //factor to keep sum stable
  ...
  for(j=0;j<npop*npar;j+=npar){
    sum[count] = 0;
    for(i=0;i<len_data;i++){
      sum[count] += fabs((y_1[i] - pop[j]*exp(pop[j+1]*x_data[i])))/factor; // |y_data - y(x)| where y(x) = a*exp(b*x)
    }
    //printf("a1: %lf b1: %lf sum1: %lf\n",pop[j],pop[j+1],sum[count]);
    count++;
  }
  ...
  fprintf(outputfile,"%d,%f,%f,%f\n",g,pop[score[0]],pop[score[0]+1],sum_pop[(int)score[0]/2]);
  fclose(outputfile);
...
void Trunc_Select(){
  ...
  for(i=0;i<nparents;i++){
    ...
    ran1 = rand() % k ;
    //printf("ran1: %d\n",ran1);
    parents[count] = pop[score[ran1]] ;
    parents[count+1] = pop[score[ran1]+1] ;
    count+=2;
...
void Interp_Cross(){
  ...
  for(i=0;i<npop*npar;i+=2){
    ...
    children[i] = (1-lambda)*parents[2*parent1] + lambda*parents[2*parent2] ;
    children[i+1] = (1-lambda)*parents[2*parent1+1] + lambda*parents[2*parent2+1] ;
  }
...
void Elitism(){
  ...
  for(j=0;j<npop*npar;j+=npar){
    sum_child[count1] = 0;
    for(i=0;i<len_data;i++){
      sum_child[count1] += fabs((y_1[i] - children[j]*exp(children[j+1]*x_data[i])))/factor; // |y_data - y(x)| where y(x) = a*exp(b*x)
      //sum = fabs((y_1[0] - pop[0]*exp(pop[1]*x_data[0])));
      //printf("a: %lf b: %lf sum: %lf\n",pop[0],pop[1],sum);
    }
    //printf("a2: %lf b2: %lf sum2: %lf\n",children[j],children[j+1],sum_child[count1]);
    count1++;
  }
  ...
  for(i=0;i<npop;i++){
    ...
    ind2 = score_all[i]+1 ;
    if(ind1<npop*npar){
      pop[count2] = children[ind1] ;
      pop[count2+1] = children[ind2] ;
    }
    else{
      pop[count2] = pop[ind1-npar*npop] ;
      pop[count2+1] = pop[ind2-npar*npop] ;
    }
    count2+=2;
  }
  // printf("Generation: %d\n",g);
